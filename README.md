...
[zipped up binary]: https://github.com/weavejester/cljfmt/releases/download/0.15.1/cljfmt-0.15.1-win-amd64.zip
...
```bash
clj -Ttools install io.github.weavejester/cljfmt '{:git/tag "0.15.1"}' :as cljfmt
```
...
```clojure
:plugins [[dev.weavejester/lein-cljfmt "0.15.1"]]
```
...
```edn
{:deps {dev.weavejester/cljfmt {:mvn/version "0.15.1"}}}
```
...
* `:remove-blank-lines-in-forms?` - whether to remove blank lines inside forms
  ...
  By default, this does not remove blank lines in pairwise constructs like
  `cond` or within binding vectors; configure these with `:blank-line-forms`
  and/or `:extra-blank-line-forms`. Defaults to false. **Experimental**.
